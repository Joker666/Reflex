...
- Discover Chromium profile names and directory identifiers for Chrome, Edge, Comet, Dia, and Helium. Read only `profile.info_cache` keys and each entry's `name` from the browser's `Local State` file. Do not read account details, history, cookies, or page data.
...
- The configured chooser modifier held when the link arrives: show the chooser, call no API, and skip the single-target bypass. Option is the default. The user can select Option, Control, Shift, Command, or Fn. This is also how the user reaches Settings when the menu bar item is hidden
...
1. **Chooser**
   ...
   - Says that automatic selection was skipped when the user held the configured modifier
